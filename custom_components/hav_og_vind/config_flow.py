...
class HavOgVindOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            vol.Required("scan_interval_minutes", default=opts.get("scan_interval_minutes", DEFAULT_SCAN_MINUTES)):
...
class HavOgVindConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            elif not (isinstance(lat, float) and isinstance(lon, float)):
                errors["base"] = "invalid_coordinates"
            else:
                unique_id = f"{round(lat, 4)}_{round(lon, 4)}"
